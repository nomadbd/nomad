...
<ProductAdminActionRow onEdit="{onEdit}" onUnhide="{onUnhide}" onUpdateStock="{onUpdateStock}" product="{product}"/>
</div>
</div>
);
};
...
.from('products')
.select(*, product_media ( media_url, media_type, sort_order ))
...
setMediaFiles(prev => prev.filter((, i) => i !== index));
};
const resetForm = () => {
...
setEditMediaFiles(prev => prev.filter((, i) => i !== index));
};
...
<style>{.minimal-input { background: transparent !important; border: none !important; border-bottom: 1px solid #262626 !important; color: #fff !important; font-size: 11px !important; outline: none !important; padding: 8px 0 !important; width: 100%; box-sizing: border-box; transition: border-color 0.2s ease; font-family: inherit; } .minimal-input:focus { border-bottom-color: #555 !important; } .minimal-input::placeholder { color: #555; }}</style>
</div>
);
}
if (editingProduct) {
...
<ProductCard key="{product.id}" onEdit="{handleOpenEdit}" onUnhide="{handleUnhideProduct}" onUpdateStock="{handleStockUpdate}" product="{product}"/>
))}
</div>
</div>
);
...
<style>{@keyframes swapFadeIn { from { opacity: 0; transform: translateY(1px); } to { opacity: 1; transform: translateY(0); } } .minimal-input { background: transparent !important; border: none !important; border-bottom: 1px solid #262626 !important; color: #fff !important; font-size: 11px !important; outline: none !important; padding: 8px 0 !important; width: 100%; box-sizing: border-box; transition: border-color 0.2s ease; font-family: inherit; } .minimal-input:focus { border-bottom-color: #555 !important; } .minimal-input::placeholder { color: #555; } .filter-expand-wrapper { display: grid; grid-template-rows: 0fr; transition: grid-template-rows 0.35s cubic-bezier(0.4, 0, 0.2, 1); overflow: hidden; } .filter-expand-wrapper.open { grid-template-rows: 1fr; } .filter-expand-content { min-height: 0; transition: opacity 0.3s ease-in-out, transform 0.3s ease-in-out; opacity: 0; transform: translateY(-8px); overflow: hidden; } .filter-expand-wrapper.open .filter-expand-content { opacity: 1; transform: translateY(0); } .search-filter-sub-wrapper { display: grid; grid-template-rows: 0fr; opacity: 0; transition: grid-template-rows 0.35s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.25s ease; overflow: hidden; } .search-filter-sub-wrapper.open { grid-template-rows: 1fr; opacity: 1; } .search-filter-sub-inner { min-height: 0; overflow: hidden; } .showroom-row-container::-webkit-scrollbar { display: none; } .showroom-row-container { -ms-overflow-style: none; scrollbar-width: none; } @media (max-width: 767px) { .admin-products-container { padding-left: 0 !important; padding-right: 0 !important; } .showroom-section { width: 100%; } .showroom-card-item { width: 100% !important; min-width: 100% !important; padding: 0 0 15px 0 !important; border-left: none !important; border-right: none !important; border-radius: 0 !important; } } @media (min-width: 768px) { .showroom-section { padding: 0; } .showroom-card-item { width: 300px; min-width: 300px; padding: 12px; margin-right: 15px; margin-bottom: 20px; } }}</style>
</div>
);
};
