...
interface OrderItem {
  ...
  quantity: number;
  price: number;
  product_image_url: string; // 📸 প্রডাক্টের থাম্বনেইল ছবি
}
      ...
      if (data) {
        const formattedOrders = data.map((order: any) => {
          const items = (order.order_items || []).map((item: any) => ({
            product_name: item.product_name || item.products?.name || 'NOMAD PREMIUM APPAREL',
            size: item.size || 'N/A',
        ...
        return (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', paddingRight: '25px' }}>
              <div>
                {/* 📄 ইনভয়েস ডাউনলোড বাটন ফিরিয়ে আনা হলো, যা সরাসরি printInvoice.ts-কে কল করবে */}
                <button 
                  onClick={(e) => {
                    if (isManageMode) {
                      toggleSelectOrder(order.id);
                      ...
                      e.stopPropagation();
                      printInvoice(order); // ⚡ এক্সটার্নাল ফাইল থেকে কল হচ্ছে
                    }
                  }}
                  style={{ 
                    background: 'none', 
            ...
            >
              {order.items.map((item, index) => (
                ...
                >
                  {/* 📸 প্রোডাক্ট থাম্বনেইল ইমেজ */}
                  <div style={{ width: '55px', height: '70px', backgroundColor: '#111', flexShrink: 0, overflow: 'hidden', border: '1px solid #222' }}>
                    {item.product_image_url ? (
                      <img src={item.product_image_url} alt={item.product_name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#444', fontSize: '9px', fontFamily: 'monospace' }}>NO IMG</div>
                    )}
                  </div>
