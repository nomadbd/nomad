...
{activeTab === 'overview' && (
<AdminOverview dateFormat="DD/MM/YYYY" key="overview" showFilter="{isFilterOpen}" userRole="{userRole}"/>
)}
{activeTab === 'orders' && (
<AdminOrders isFilterOpen="{isFilterOpen}" isSearchOpen="{isSearchOpen}" key="orders" onToggleSearch="{()"> setIsSearchOpen(prev => !prev)}
...
{activeTab === 'products' && (
<AdminProducts dateFormat="DD/MM/YYYY" isFilterOpen="{isFilterOpen}" key="products" searchQuery="{searchQuery}"/>
)}
{activeTab === 'settings' && (
isSuperAdmin ? (
...
<StaffProfile isOpen="{isProfileOpen}" onClose="{()"> setIsProfileOpen(false)}
profile={profileData}
