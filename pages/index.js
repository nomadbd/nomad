...
export default function Home({ allProducts, siteContent, announcement }) {
  ...
  const [modalType, setModalType] = useState(null); // 'details', 'order', 'cart'
  const [paymentMethod, setPaymentMethod] = useState('');
  const [viewCategory, setViewCategory] = useState(null);

  const paymentNumbers = { 'Bkash': '01521731371', 'Nagad': '01521731371', 'Rocket': '01521731371', 'Upay': '01521731371', 'Cellfin': '01521731371' };
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', fontFamily: 'Inter, sans-serif', overflowX: 'hidden' }}>
      ...
      <header style={{ position: 'relative', textAlign: 'center', padding: '40px 20px' }}>
        <h1 style={{ letterSpacing: '12px', fontSize: '24px', fontWeight: '900', margin: 0 }}>NOMAD</h1>
        <div onClick={() => setModalType('cart')} style={{ position: 'absolute', right: '20px', top: '40px', fontSize: '10px', cursor: 'pointer', border: '1px solid #333', padding: '8px 12px', borderRadius: '20px' }}>
          BAG ({cart.length})
        </div>
                ...
                <div key={i} className="cat-item">
                  ...
                  <div style={{ padding: '25px 20px', textAlign: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      ...
                      <button className="btn-style" onClick={() => addToCart({ ...p, priceText: p.priceText })}>ADD TO BAG</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {modalType === 'cart' && <CartModal onClose={closeModal} />}
      {(selectedProduct && modalType === 'details') && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#0a0a0a', width: '100%', maxWidth: '400px', padding: '40px 25px', borderRadius: '40px', border: '1px solid #1a1a1a' }}>
            ...
            <button className="btn-style" style={{ width: '100%' }} onClick={() => { addToCart({ ...selectedProduct }); closeModal(); }}>ADD TO BAG</button>
            <p onClick={closeModal} style={{ textAlign: 'center', color: '#666', marginTop: '20px', cursor: 'pointer' }}>CANCEL</p>
          </div>
        </div>
      )}
    </div>
...
export async function getStaticProps() {
  ...
  const readTxt = (file, def) => { const p = path.join(cDir, file); return fs.existsSync(p) ? fs.readFileSync(p, 'utf8').trim() : def; };
  ...
  return { props: { allProducts, siteContent: { header: readTxt('header.txt', 'NOMAD'), footer: readTxt('footer.txt', '2026') }, announcement: readTxt('announcement.txt', '') }, revalidate: 10 };
}
