    ...
    <div className="bg-zinc-900 border border-zinc-800 w-full overflow-hidden mb-8 rounded-xl shadow-lg">
      ...
      <div className="p-6">
        ...
        <p className="text-zinc-400 text-sm leading-relaxed mb-4">
          ...
          <span onClick={() => setIsExpanded(!isExpanded)} className="text-zinc-500 cursor-pointer ml-1 hover:text-zinc-300">
        ...
        {isExpanded && details && (
          <div className="mb-6 bg-zinc-950 p-4 rounded-lg border border-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm text-zinc-300">
              {Object.entries(details).map(([key, value]: any, i) => (
        ...
        <div className="flex items-center justify-between pt-6 border-t border-zinc-800">
          ...
          <button onClick={() => setIsFormOpen(true)} className="bg-white text-black px-8 py-3 uppercase font-bold text-xs tracking-widest hover:bg-zinc-200 transition-all rounded-full">
            ORDER NOW
          </button>
        </div>
      </div>
      ...
      {isFormOpen && (
        <OrderForm 
          product={{ title, price, id, deliveryCharge, vat }} 
